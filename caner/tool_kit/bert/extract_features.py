...
def model_fn_builder(bert_config, init_checkpoint, use_one_hot_embeddings=False,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        for k in range(1, -pooling_layer+1):
            temp_list.append(-k)

        all_layers = []
        if len(temp_list) == 1:
            encoder_layer = model.all_encoder_layers[temp_list[-1]]
        else:
...
def convert_lst_to_features(lst_str, seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(read_examples(lst_str)):
        ...
        if tokens_b:
            ...
            assert len(tokens_a) <= seq_length - 2, '分词后长度超过seq_length: '+str(len(tokens_a))+' / '+str(seq_length - 2)
